...
async def generate_final_image(request: ImageGenerationRequest):
    ...
    Uses Replicate's Stable Diffusion to place furniture in the room
    ...
    try:
        ...
        logger.info("📥 Downloading original room image...")
        ...
        raise HTTPException(
            ...
            detail="Failed to download room image. Please try uploading again."
        )
...
async def regenerate_image(session_id: str, new_prompt: str):
    ...
    logger.info(f"🔄 Regenerating image with new prompt...")
...
@router.get(
    ...
    description="Get all generated images for a session"
)
async def get_generation_history(session_id: str):
    ...
    return {
        ...
        "message": f"Found {len(session.generated_images)} generated images"
    }